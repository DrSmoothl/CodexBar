...
enum HomebrewUpdateError: LocalizedError, Equatable {
    ...
    var errorDescription: String? {
        ...
        case let .versionUnchanged(version):
            "Homebrew finished, but CodexBar is still version \(version)."
        }
    }
}

/// Keeps Homebrew as the owner of cask installs: it only reads the tap's cask version and runs
/// `brew upgrade` on request, so Homebrew's recorded version always matches the installed app.
@MainActor
@Observable
final class HomebrewUpdaterController: UpdaterProviding {
    ...
    func performCheck() async {
        ...
        do {
            ...
            self.phase = self.updateStatus.availableVersion.map(Phase.available) ?? .idle
        }
    }
    ...
    func performInstall() async {
        ...
        do {
            ...
            guard HomebrewCaskVersion.isNewer(installedVersion, than: startingVersion) else {
...
extension HomebrewUpdaterController.Dependencies {
    ...
    static var live: Self {
        ...
        return Self(
            ...
            runUpgrade: {
                guard let brew = Self.brewCandidates.first(where: FileManager.default.isExecutableFile(atPath:)) else {
                ...
                _ = try await SubprocessRunner.runToCompletion(
                    ...
                    environment: Self.brewEnvironment(brewPath: brew),
    ...
    private static func relaunch(bundleURL: URL) {
        ...
        process.arguments = ["-c", "sleep 1; /usr/bin/open -n \"$0\"", bundleURL.path]
        do {
            try process.run()
            NSApp.terminate(nil)
        } catch {
